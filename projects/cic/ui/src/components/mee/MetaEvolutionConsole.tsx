// File: projects/cic/ui/src/components/mee/MetaEvolutionConsole.tsx | Date: 2026-06-03 | v1.0.0
...
export function MetaEvolutionConsole() {
  ...
  const [patchDetails, setPatchDetails] = useState<{ proposalId: string; patches: any[] } | null>(null);
  ...
  const triggerProposalScan = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setMessage(`Scan complete. Found ${data.events?.length || 0} trigger events.`);
      fetchProposals();
    } catch (err: any) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {selectedProposalId ? (
            <div>
              {proposals.filter(p => p.id === selectedProposalId).map(prop => (
                <div key={prop.id}>
                  ...
                  {/* Skeletons to Create */}
                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ fontSize: "1rem", color: "#9ca3af", fontWeight: 700, marginBottom: "8px" }}>Target Files Created</h3>

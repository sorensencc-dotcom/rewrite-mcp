...
export class MeeResearchEngine {
  constructor(
    private readonly findingsStore: FileMeeResearchFindingStore,
    private readonly metaRulesStore: FileMeeMetaRuleStore,
    private readonly runStore: FileMeeRunStore,
    private readonly failureStore: FileMeeRunFailureContextStore,
    private readonly llmClient?: {
      complete(options: { model: string; prompt: string; max_tokens?: number }): Promise<{ text: string }>;
    }
  ) {}

  async runResearchScan(kg: MeeKnowledgeGraph): Promise<{ findings: ResearchFinding[]; rules: MeeMetaRule[] }> {
    ...
    let synthesizedFindings: ResearchFinding[] = [];
    let synthesizedRules: MeeMetaRule[] = [];
      ...
      try {
        ...
        if (match) {
          ...
          if (Array.isArray(parsed.findings)) {
            synthesizedFindings = parsed.findings.map((f: any) => ({
          ...
          if (Array.isArray(parsed.rules)) {
            synthesizedRules = parsed.rules.map((r: any) => ({

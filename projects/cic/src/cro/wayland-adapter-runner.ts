...
import { SecurityPolicy, SecurityConfig } from '../../wil/src/security/SecurityPolicy.js';
...
export class WaylandAdapterRunner implements AgentRunner {
  ...
  public async run(task: TaskExecution, isDryRun: boolean): Promise<any> {
    if (isDryRun) {
      return {
        ok: true,
        dryRun: true,
  ...
  private async executeAdapterOperation(op: any, task: TaskExecution): Promise<any> {
    switch (op.type) {
      case 'shell':

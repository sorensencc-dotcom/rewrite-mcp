...
import { ConditionalEvaluationError } from "./errors.js";
...
export class ConditionalEvaluator {
  ...
  evaluate(template: string, vars: Record<string, any>): string {
    ...
    while (innerRegex.test(result)) {
      if (iterations++ > maxIterations) {
        throw new ConditionalEvaluationError("Excessive nesting or malformed conditional loop detected");
      }
      ...
      result = result.replace(innerRegex, (match, condExpr, content) => {
        try {
          ...
          throw new ConditionalEvaluationError(`Failed to evaluate condition expression '${condExpr}': ${err.message}`);
        }
      });
    }
   ...
   */
  private evaluateExpression(expr: string, vars: Record<string, any>): boolean {
    if (!expr) {
      return false;
    }

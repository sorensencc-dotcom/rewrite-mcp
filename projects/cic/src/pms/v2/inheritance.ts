...
export class InheritanceResolver {
  ...
  resolve(
    template: TemplateV2,
    allTemplates: Map<string, TemplateV2>,
    visited: Set<string> = new Set<string>()
  ): string {
    const tid = template.template_id;

    if (visited.has(tid)) {
      throw new InheritanceResolutionError(
        `Circular template inheritance detected: ${Array.from(visited).join(" -> ")} -> ${tid}`
      );
    }
    visited.add(tid);
    ...
    if (!parentTemplate) {
      throw new InheritanceResolutionError(
        `Parent template '${template.parent}' not found in registry for template '${tid}'`
      );
    }

...
/**
 * CIC Telemetry Event (raw format from emitter)
 */
interface RawCicEvent {
  type: string;
  timestamp: string;
  agent?: string;
  model?: string;
...
export function loadCicLlmEvents(): CodeburnEvent[] {
  ...
  try {
    ...
    return lines.map((line) => {
      const raw = JSON.parse(line) as RawCicEvent;
      return {
 ...
 */
export function loadCicCostEvents(): CodeburnEvent[] {
...
export function aggregateToModelStats(events: CodeburnEvent[]): ModelStats[] {
  const byModel = new Map<string, CodeburnEvent[]>();
  ...
  byModel.forEach((events, key) => {
    ...
    const avgCost =
      events.reduce((sum, e) => sum + (e.metadata.total_cost_usd as number) || 0, 0) / Math.max(events.length, 1);
    const avgRetry =
      events.reduce((sum, e) => sum + ((e.metadata.retry_count as number) || 0), 0) / Math.max(events.length, 1);

    const stages = [...new Set(events.map((e) => e.metadata.pipeline_stage as string).filter(Boolean))];
    const agents = [...new Set(events.map((e) => e.metadata.agent as string).filter(Boolean))];
 ...
 */
export function exportCicTelemetry(outputPath?: string): void {

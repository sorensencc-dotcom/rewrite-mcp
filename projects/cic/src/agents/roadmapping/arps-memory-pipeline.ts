...
import { RoadmapHarvester, RoadmapDelta } from './harvester-agent.js';
...
export class ArpsMemoryPipeline {
  ...
  private memoryHarvester: MemoryHarvester;
   ...
   */
  private async emitArpsDelta(delta: RoadmapDelta): Promise<void> {
    try {
  ...
  private queryMemoryContext(): {
    ...
    try {
      ...
      let trend = 'stable';
      if (recentCount > 5) trend = 'improving';
      if (recentCount === 0) trend = 'stable';
  ...
  async run(opts: ArpsMemoryPipelineOptions): Promise<void> {
    ...
    if (opts.verbose) console.log(`[ARPS Pipeline] Querying memory for context...`);
    ...
    if (opts.verbose) console.log(`[ARPS Pipeline] Emitting ARPS_DELTA to memory layer...`);

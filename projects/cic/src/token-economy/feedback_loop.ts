...
/**
 * Model Statistics exported by CodeBurn
 * Used to drive routing rule updates.
 */
interface CodeburnModelStats {
  model: string;
  avg_cost_usd: number;
...
/**
 * Routing Rule
 * Defines how to route calls to models based on pipeline stage.
 */
...
const EXPORT_PATH =
  process.env.CODEBURN_EXPORT_PATH ||
 ...
 */
function loadCodeburnStats(): CodeburnModelStats[] {
...
/**
 * Generate recommendations based on CodeBurn insights
 */
...
function generateRecommendations(stats: CodeburnModelStats[]): Recommendation[] {
  const recommendations: Recommendation[] = [];

  // Group stats by pipeline stage
  const byStage = new Map<string, CodeburnModelStats[]>();
  stats.forEach((stat) => {
 ...
 */
export function applyCodeburnInsights(): void {
  ...
  fs.writeFileSync(
    ROUTING_RULES_PATH,
    JSON.stringify({ version: "1.0.0", rules: updatedRules, updated_at: new Date().toISOString() }, null, 2),
    "utf8"
  );
 ...
 */
function updateRulesFromRecommendations(rules: RoutingRule[], recommendations: Recommendation[]): RoutingRule[] {
  const updated = [...rules];
  ...
  recommendations.forEach((rec) => {
    ...
    if (ruleIndex >= 0 && rec.confidence >= 0.8) {
      console.log(
        `[TokenEconomy] Updating ${rec.stage}: ${rec.current_model} → ${rec.recommended_model} (${rec.reason})`
      );
 ...
 */
export function exportTelemetryForCodeburn(): void {

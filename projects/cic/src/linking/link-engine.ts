...
import { SemanticDocument, TopicVector } from "../harvester/extractors/v2/extractor-v2.types.js";
...
export class LinkEngine {
  computeLinks(doc: SemanticDocument, allDocs: SemanticDocument[]): CrossDocumentLink[] {
    ...
    const addLink = (link: Omit<CrossDocumentLink, "id">) => {
    ...
    const getTopicVectors = (document: SemanticDocument): TopicVector[] => {
    ...
    for (const other of allDocs) {
      if (other.docId === doc.docId) continue;
      ...
      const sharedEntities: { thisEnt: any; otherEnt: any }[] = [];

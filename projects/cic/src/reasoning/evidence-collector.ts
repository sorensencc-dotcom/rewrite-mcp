...
import { GraphBuilder, graphBuilder } from "../linking/graph-builder.js";
...
export interface RankedEvidence {
  ...
  score: number;
  text: string;
  provenance: string; // source doc ID or path
  timestamp: string;
}
...
export class EvidenceCollector {
  ...
  async collect(plan: RetrievalPlan): Promise<RankedEvidence[]> {
    const evidence: RankedEvidence[] = [];
    const seenIds = new Set<string>();

    // 1. Determine active graph scope
    let activeGraph: GraphBuilder = graphBuilder;
    
    if (plan.temporalSlice) {
      // Reconstruct historical sliced graph at dateX dynamically!
      const slice = graphBuilder.sliceAtDate(plan.temporalSlice);
      const tempGraph = new GraphBuilder();
      for (const doc of slice.documents) {
        // Find links associated with this document from the slice
        const docLinks = slice.crossDocLinks.filter(
          l => l.sourceDocId === doc.docId || l.targetDocId === doc.docId
        );
    ...
    for (const gq of plan.graphQueries) {
      try {
        ...
        for (const doc of neighborhood.documents) {
          if (seenIds.has(doc.docId)) continue;
        ...
        for (const rel of neighborhood.relationships) {
          ...
          if (seenIds.has(relKey)) continue;
          seenIds.add(relKey);

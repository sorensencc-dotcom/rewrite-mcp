...
export interface RetrievalPlan {
  query: string;
  vectorQueries: { query: string; limit: number }[];
  graphQueries: { startEntityId: string; depth: number; edgeTypes?: string[] }[];
  evidenceBudget: { maxDocuments: number; maxTokens: number };
  temporalSlice?: string;
}

export class RetrievalPlanner {
  plan(query: string, context?: { timeWindow?: string; maxDocuments?: number; maxTokens?: number }): RetrievalPlan {
    const vectorQueries: { query: string; limit: number }[] = [];
    const graphQueries: { startEntityId: string; depth: number; edgeTypes?: string[] }[] = [];

...
import { CkgStore, CkgGraph, CkgNode, CkgEdge } from "./ckg-store.js";

export class CkgSynthesizer {
  constructor(private store: CkgStore) {}

  public run(): void {
    const graph = this.store.load();

    // 1. Deduplicate nodes & edges
    const uniqueNodesMap = new Map<string, CkgNode>();
    for (const node of graph.nodes) {
      uniqueNodesMap.set(node.id, node);
    }
    const dedupedNodes = Array.from(uniqueNodesMap.values());

    const uniqueEdgesMap = new Map<string, CkgEdge>();
    ...
    const centralNodes: CkgNode[] = [];
    const orphans: CkgNode[] = [];
    ...
    const unmappedSkills: any[] = [];
    const stateDiscrepancies: any[] = [];

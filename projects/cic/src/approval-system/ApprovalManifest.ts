...
const DEFAULT_MANIFEST_PATH =
  process.env.CIC_APPROVAL_MANIFEST ||
...
const DEFAULT_CONFIG = {
  ...
  trustScoreBias: "moderate" as const,
};

export class ApprovalManifestManager {
  private manifest: ApprovalManifest;
  ...
  private createEmpty(): ApprovalManifest {
    return {
      version: "2.0.0",
  ...
  async load(): Promise<void> {
    try {
      ...
      if (parsed.version !== "2.0.0") {
        console.warn(
          `[ApprovalManifest] Version mismatch: ${parsed.version} vs 2.0.0`
        );
      }
  ...
  async recordApproval(
    command: string,
    reason?: string
  ): Promise<{ promoted: boolean; newTier?: ApprovalTier }> {
    ...
    if (!record) {
      ...
      record.trustScore = this.computeTrustScore(
        record.tier,
        record.approvalCount,
        record.rejectCount
      );
    ...
    let promoted = false;
    let newTier: ApprovalTier | undefined;

    if (
      record.approvalCount % DEFAULT_CONFIG.autoPromoteThreshold === 0 &&
      record.tier < 4
    ) {
      newTier = (record.tier + 1) as ApprovalTier;
      record.tier = newTier;
   ...
   */
  async recordRejection(command: string): Promise<void> {
    const commandId = this.normalizeCommandId(command);
    const now = new Date().toISOString();
   ...
   */
  async addToSessionWhitelist(command: string, reason?: string): Promise<void> {
    const commandId = this.normalizeCommandId(command);
    const now = new Date().toISOString();
    const expiresAt = new Date(
      Date.now() + DEFAULT_CONFIG.sessionWhitelistTtlMs
    ).toISOString();
  ...
  private checkDenylist(
    command: string
  ): { pattern: DenylistPattern; severity: "low" | "medium" | "high" } | null {
   ...
   */
  private normalizeCommandId(command: string): string {
   ...
   * Clean expired session whitelist entries
   */
  private cleanExpiredSessionWhitelist(): void {
    const now = new Date();
  ...
  private updateStats(): void {
    ...
    for (const record of Object.values(this.manifest.approvals)) {
      if (record.tier === 1) stats.tier1Count++;
      else if (record.tier === 2) stats.tier2Count++;
      else if (record.tier === 3) stats.tier3Count++;
      else if (record.tier === 4) stats.tier4Count++;
    }
   ...
   */
  listAutoPromotedCommands(sinceMs: number = 3600000): ApprovalRecord[] {
    const sinceTime = new Date(Date.now() - sinceMs);
    return Object.values(this.manifest.approvals).filter(
      (r) =>
        r.autoPromotedAt &&

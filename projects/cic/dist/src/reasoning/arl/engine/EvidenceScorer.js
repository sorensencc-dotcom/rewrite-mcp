export function scoreEvidence(packet, hypotheses) {
    const { context } = packet;
    return hypotheses.map(hypothesis => ({
        hypothesis,
        narrativeContinuity: scoreNarrativeContinuity(hypothesis, context.narrativeSpine),
        semanticCoherence: scoreSemanticCoherence(hypothesis, context.semanticMap),
        timelineConsistency: scoreTimelineConsistency(hypothesis, context.recentExpansions),
        driftPenalty: scoreDriftPenalty(hypothesis, context.driftScore),
        stabilityAlignment: scoreStabilityAlignment(hypothesis, context.stabilityMetrics)
    }));
}
function scoreNarrativeContinuity(hypothesis, spine) {
...
function scoreSemanticCoherence(hypothesis, semanticMap) {
...
function scoreTimelineConsistency(hypothesis, recentExpansions) {
...
function scoreDriftPenalty(hypothesis, driftScore) {
...
function scoreStabilityAlignment(hypothesis, stabilityMetrics) {

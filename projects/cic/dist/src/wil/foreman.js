...
export class CICForeman {
    constructor() {
        ...
        this.port = 3035;
        this.host = '127.0.0.1';
        this.registryUrl = process.env.WAYLAND_REGISTRY_URL || 'http://127.0.0.1:4000';
        this.agentId = 'cic-foreman-wil-v1';
    ...
    async registerWithWaylandRegistry() {
        try {
    ...
    makeHttpRequest(method, url, body) {
    ...
    handlePostTask(req, res, correlationId, startTime) {
        let body = '';
    ...
    handleGetStatus(res, taskId, correlationId, startTime) {
    ...
    handleGetArtifact(res, taskId, artifactId, correlationId, startTime) {
        const task = this.tasks.get(taskId);
        if (!task) {
            res.writeHead(404);
            res.end(JSON.stringify({ error: 'Task not found' }));
    ...
    handleWriteArtifact(req, res, taskId, correlationId, startTime) {
        const task = this.tasks.get(taskId);
        if (!task) {
            res.writeHead(404);
            res.end(JSON.stringify({ error: 'Task not found' }));
        ...
        req.on('end', async () => {
            try {
                const metadata = req.headers['x-artifact-metadata'] ? JSON.parse(req.headers['x-artifact-metadata']) : {};
                const artifactMetadata = await this.artifactManager.writeArtifact(taskId, metadata.name || 'artifact', body, metadata.mime_type || 'application/octet-stream', metadata.tags);
                if (!artifactMetadata) {
                    res.writeHead(413);
    ...
    handleListArtifacts(res, taskId, correlationId, startTime) {
        const task = this.tasks.get(taskId);
        if (!task) {
            res.writeHead(404);
            res.end(JSON.stringify({ error: 'Task not found' }));
    ...
    handleHealth(res, correlationId, startTime) {
        const health = {
            status: 'healthy',
            timestamp: new Date().toISOString(),
    ...
    handleCreateSession(req, res, correlationId, startTime) {
        let body = '';
        req.on('data', (chunk) => { body += chunk; });
        req.on('end', () => {
            try {
    ...
    handleGetSession(res, sessionId, correlationId, startTime) {
    ...
    handleSessionEvent(req, res, sessionId, correlationId, startTime) {
        const session = this.sessionMapper.getSession(sessionId);
        if (!session) {
            res.writeHead(404);
            res.end(JSON.stringify({ error: 'Session not found' }));
    ...
    handleGetSessionStats(res, sessionId, correlationId, startTime) {
    ...
    handleEventsStream(req, res, correlationId) {
        res.writeHead(200, {

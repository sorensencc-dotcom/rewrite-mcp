...
export class SecurityPolicy {
  ...
  private initializeFromConfig(config: SecurityConfig) {
    if (config.shell) {
  ...
  enforceFilePath(path: string, isWrite: boolean = false): EnforcementResult {
  ...
  enforceModelOptions(options: { maxTokens?: number; temperature?: number }): EnforcementResult {
  ...
  private enforceRule(type: 'shell' | 'file' | 'http' | 'model' | 'browser', value: string): EnforcementResult {

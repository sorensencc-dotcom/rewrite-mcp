...
const createPhaseDAgets = () => {
  return {
    "code-analyzer": {
      invoke: async (method: string, input: any) => ({
        agent: "code-analyzer",
        method,
        status: "success",
...
async function runPhaseDZeroRun() {
  ...
  try {
    ...
    const mainPipelineExists = flows.some((f: any) => f.id === "cic-main-pipeline");
    ...
    expectedStages.forEach((stageId) => {
      ...
      if (status === "completed") stagesGreen++;
    });
    ...
    const diagnosticsSpan = execution.spans?.find((s: any) => s.stage_id === "stage-6");
    ...
    console.log(`   Trace ID consistent: ${spans.every((s: any) => s.trace_id === traceId) ? "✓" : "✗"}`);
    console.log(`   No orphans: ${spans.every((s: any) => !s.parent_span_id || spans.some((p: any) => p.span_id === s.parent_span_id)) ? "✓" : "✗"}`);
    console.log();

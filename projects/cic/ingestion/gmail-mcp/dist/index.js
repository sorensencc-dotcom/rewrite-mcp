...
server.registerTool('list_unread_emails', {
    description: 'Fetch unread emails from the last N hours. Returns email metadata including sender, subject, date, and labels.',
    inputSchema: z.object({
        maxResults: z.number().int().min(1).max(100).default(50).describe('Maximum number of emails to return (default: 50, max: 100)'),
        hoursBack: z.number().int().min(1).default(24).describe('How many hours back to search (default: 24)')
    })
...
server.registerTool('read_email', {
    description: 'Get the full content of an email including headers, body (plain text and HTML), and labels.',
    inputSchema: z.object({
...
// Tool: Apply labels
server.registerTool('apply_labels', {
    description: 'Add labels to one or more emails. Creates labels if they do not exist.',
    inputSchema: z.object({
...
server.registerTool('search_emails', {
    description: 'Search emails using Gmail query syntax (e.g., "from:someone@example.com", "subject:urgent", "is:unread").',
    inputSchema: z.object({
        query: z.string().describe('Gmail search query (e.g., "from:example.com is:unread", "subject:invoice after:2024-01-01")'),
        maxResults: z.number().int().min(1).max(100).default(50).describe('Maximum results to return (default: 50, max: 100)')
    })
...
server.registerTool('analyze_email', {
    description: 'Analyze email content to categorize it as ACTION_REQUIRED, NEWSLETTER, PROMOTION, or NOISE. Useful for email triage automation.',
    inputSchema: z.object({

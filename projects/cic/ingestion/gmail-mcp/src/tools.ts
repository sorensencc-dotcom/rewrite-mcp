...
export class GmailTools {
  ...
  async listUnreadEmails(
    ...
    try {
      ...
      throw new Error(
        `Failed to list unread emails: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private async getEmailMetadata(messageId: string): Promise<EmailMetadata | null> {
    try {
   ...
   */
  async readEmail(messageId: string): Promise<EmailContent | null> {
    try {
      const response = await this.gmail.users.messages.get({
        userId: 'me',
        id: messageId,
      ...
      throw new Error(
        `Failed to read email: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * Create a new label
   */
  private async createLabel(labelName: string): Promise<Label> {
    try {
      ...
      return {
        ...
        type: (response.data.type as 'user' | 'system') || 'user'
      };
    } catch (error) {
      throw new Error(
   ...
   */
  async analyzeEmail(messageId: string): Promise<AnalysisResult> {
    try {
      ...
      let category: 'ACTION_REQUIRED' | 'NEWSLETTER' | 'PROMOTION' | 'NOISE' =
        'NOISE';
      let confidence = 0.5;
      let reasoning = '';

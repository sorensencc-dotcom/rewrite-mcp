import { HealthCheck, SelfDiagnosticsResult, ContradictionDetectorResult } from '../contracts/HealthMetrics';

export interface SubsystemState {
  compositeReasoning: { score: number; variance: number };
  confidenceModel: { score: number; variance: number };
  driftCalculator: { score: number; variance: number };
  verdictSynthesizer: { score: number; variance: number };
}

export class SelfDiagnosticsEngine {
  private lastState: SubsystemState | null = null;
  private stateHistory: SubsystemState[] = [];
  ...
  runDiagnostics(currentState: SubsystemState): SelfDiagnosticsResult {
    ...
    const reasoningIntegrityScore = this.computeIntegrityScore(
      driftOfDriftScore,
    ...
    const overallStatus =
      checks.every((c) => c.status === 'healthy') ? 'healthy' : checks.some((c) => c.status === 'failed') ? 'failed' : 'degraded';
  ...
  private checkSubsystemHealth(state: SubsystemState): HealthCheck {
    const scores = [
  ...
  private calculateDriftOfDrift(): number {
  ...
  private validateWeightingSanity(state: SubsystemState): number {
    const weights = [
      state.compositeReasoning.score,
      state.confidenceModel.score,
      state.driftCalculator.score,
    ...
    if (hasExtremeWeight) return 0.4;
  ...
  private detectInternalContradictions(state: SubsystemState): ContradictionDetectorResult {
    const contradictions: ContradictionDetectorResult['contradictions'] = [];
  ...
  private computeIntegrityScore(
    driftOfDrift: number,
    weightingSanity: number,
    contradictions: ContradictionDetectorResult
  ): number {
  ...
  private generateRecommendations(checks: HealthCheck[], integrityScore: number): string[] {

...
import {
  MultiRunAggregate,
  StabilityTrend,
} from '../contracts/MultiRunAggregate';
...
export class MultiRunAggregator {
  aggregate(runs: RunSummary[]): MultiRunAggregate {
    ...
    const rollingContradictionAverage = this.average(
      runs.map((r) => r.contradictionScore)
    );
    const rollingConfidenceAverage = this.average(
      runs.map((r) => r.confidenceScore)
    );
    const rollingCompositeAverage = this.average(
  ...
  appendRun(
    _existing: MultiRunAggregate,
    _newRun: RunSummary
  ): MultiRunAggregate {
    throw new Error(
      'appendRun not implemented: requires run history or incremental formulas'
    );
  }
  ...
  private computeTrend(driftSeries: number[]): StabilityTrend {
    if (driftSeries.length < 2) return 'STABLE';
  ...
  private computeStabilityScore(
    ...
    let score =
      confidenceBonus - (driftPenalty + contradictionPenalty) / 2;

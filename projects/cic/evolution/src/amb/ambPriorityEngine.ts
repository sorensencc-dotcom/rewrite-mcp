...
export interface AmbSignals {
  drift_metrics?: { tenant_drift_index?: number };
  distillation_stats?: { stale_node_ratio?: number; redundant_node_ratio?: number };
  mas_health?: { agent_consensus_rate?: number; critique_count?: number };
  rl_metrics?: { average_lighthouse_improvement?: number; conversion_rate?: number };
}
...
export class AmbPriorityEngine {
  public computePriorities(signals: AmbSignals): AmbPriorityResult[] {
    const priorities: AmbPriorityResult[] = [];

...
import { CkgStore, CkgGraph, CkgNode, CkgEdge } from "../../src/ckg/ckg-store.js";
...
export class KnowledgeDistillationEngine {
  ...
  public runDistillation(stalenessThresholdMs: number = 14 * 24 * 60 * 60 * 1000): {
    ...
    const now = Date.now();
    const staleNodes: CkgNode[] = [];
    const redundantGroups: CkgNode[][] = [];
    const processedRedundant = new Set<string>();
    ...
    const compressedNodes: CkgNode[] = [];
    const compressedEdges: CkgEdge[] = [];
    const mergeMap = new Map<string, string>(); // source -> target
    ...
    for (const edge of originalEdges) {
      ...
      const edgeExists = compressedEdges.some(
        e => e.from === mappedFrom && e.to === mappedTo && e.type === edge.type
      );
      if (!edgeExists) {

...
// 3. Spies and Mocks
vi.spyOn(rtkCic, "submitIngestionJob").mockImplementation(async (job) => {
    try {
...
vi.spyOn(PMSExecutor.prototype, "execute").mockImplementation(function (reqOrTemplateId, vars) {
...
vi.spyOn(cicGitai, "generateGovernanceDelta").mockImplementation((data) => {
    const result = {
...
vi.spyOn(sectionTracking, "advanceSection").mockImplementation((section, state) => {
...
export function snapshotAutomationState(orchestrator) {
...
export function getMetricsSnapshot(orchestrator) {

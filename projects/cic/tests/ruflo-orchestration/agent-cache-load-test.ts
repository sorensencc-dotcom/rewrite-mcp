...
describe("Agent Cache Load Test — 10 Parallel Flows", () => {
  it("measures cache hit rate and latency improvement", async () => {
    ...
    const cache = new MemoryAgentCache(1000, 60000); // 1000 capacity, 60s TTL
    const cachedAgent = new CachedAgentClient({
      agent: slowAgent,
      cache,
      ttl: 60000,
    });
    ...
    console.log(
      `Avg cached invocation time: ${(invocationTimes.filter((_, i) => stats.hits > 0 && i > 2).reduce((a, b) => a + b, 0) / Math.max(1, stats.hits)).toFixed(1)}ms`
    );

    // Assertions
    // Expected: 5 unique inputs = 5 invocations, 5 cache hits from duplicates
    expect(totalInvocations).toBeLessThanOrEqual(5); // Should only invoke 5 times
    expect(stats.hits).toBeGreaterThanOrEqual(5); // Should have at least 5 hits
    expect(cacheHitRate).toBeGreaterThan(0.5); // >50% hit rate
    expect(results.length).toBe(10); // All flows completed
  });

  it("measures latency improvement: cached vs uncached", async () => {
    ...
    // Test cached
    const cache = new MemoryAgentCache(100, 60000);
    const cachedAgent = new CachedAgentClient({
      agent: slowAgent,
      cache,
      ttl: 60000,
    ...
    const latencyImprovement =
      ((uncachedTotalTime - cachedTotalTime) / uncachedTotalTime) * 100;

    console.log("\n=== Latency Comparison ===");
    console.log(`Uncached (5 calls): ${uncachedTotalTime}ms`);
    console.log(`Cached (5 calls): ${cachedTotalTime}ms`);
    console.log(
      `Latency improvement: ${latencyImprovement.toFixed(1)}% faster`
    );

    // Cached should be significantly faster (first call ~100ms, next 4 calls ~0-5ms)
    expect(cachedTotalTime).toBeLessThan(uncachedTotalTime);
    expect(latencyImprovement).toBeGreaterThan(50); // At least 50% faster
  });
});

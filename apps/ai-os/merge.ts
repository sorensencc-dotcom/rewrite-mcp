...
async function mergeUnifiedMemory(root: string) {
  ...
  const unifiedMemoryPath = path.join(root, "MEMORY", "unified_memory.md");
  await fs.mkdir(path.dirname(unifiedMemoryPath), { recursive: true });
  await fs.writeFile(unifiedMemoryPath, "# Unified Memory\n\n- Placeholder content", "utf8");
}

async function generateMemoryContract(root: string) {
    ...
    const toYAMLString = (key: string, obj: any) => {
        let yaml = `${key}:\n`;
    ...
    await fs.writeFile(contractPath, mdContent, "utf8");
}

export async function merge(root: string) {
...
async function generateMetaEvolutionRanker(root: string) {
    ...
    await fs.mkdir(path.dirname(rankerPath), { recursive: true });
...
`;
    ...
    await fs.writeFile(rankerPath, content.trim(), "utf8");
}

async function generateMetaEvolutionSimulator(root: string) {
    console.log("Generating meta-evolution simulator...");
    const simulatorPath = path.join(root, "SYSTEM", "meta_evolution_simulator.md");
    await fs.mkdir(path.dirname(simulatorPath), { recursive: true });
...
`;
    ...
    await fs.writeFile(simulatorPath, content.trim(), "utf8");
}

async function generateMetaEvolutionLog(root: string) {
    ...
    let existingContent = "";
    try {
        existingContent = await fs.readFile(logPath, "utf8");
    } catch {
...
async function generateMetaEvolutionEngine(root: string) {
...
`;
    ...
    await fs.writeFile(enginePath, content.trim(), "utf8");
}

async function generateMetaAuditEngine(root: string) {
...
`;
    ...
    await fs.writeFile(auditPath, content.trim(), "utf8");
}

async function generateMetaOperatorLayer(root: string) {
    console.log("Generating meta-operator layer...");
    const molPath = path.join(root, "SYSTEM", "meta_operator_layer.md");
    await fs.mkdir(path.dirname(molPath), { recursive: true });
...
`;
    ...
    await fs.writeFile(molPath, content.trim(), "utf8");
}

async function generatePolicyEngine(root: string) {
...
`;
    ...
    await fs.writeFile(policiesPath, content.trim(), "utf8");
}

async function generateOperatorControlPlane(root: string) {
...
`;
    ...
    await fs.writeFile(cpPath, content.trim(), "utf8");
}

async function generateExecutionModel(root: string) {
...
`;
    ...
    await fs.writeFile(modelPath, content.trim(), "utf8");
}

async function generateAgentOrchestrationContract(root: string) {
...
`;
    ...
    await fs.writeFile(contractPath, content.trim(), "utf8");
}

async function generateCoherenceLayer(root: string) {
    console.log("Generating coherence layer...");
    const coherenceLayerPath = path.join(root, "SYSTEM", "coherence_layer.md");
    await fs.mkdir(path.dirname(coherenceLayerPath), { recursive: true });
    ...
    try {
        ...
        for (const file of workflowFiles) {
            workflowContent.push(path.parse(file).name);
        }
    } catch {
        // dir may not exist
    }
...
`;
    ...
    await fs.writeFile(coherenceLayerPath, content.trim(), "utf8");
}
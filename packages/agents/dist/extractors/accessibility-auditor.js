...
export class AccessibilityAuditor {
     ...
     */
    audit(domModel, computedStyles, targetLevel = 'AA') {
    ...
    analyzeMetrics(domModel) {
        const headingHierarchy = new Map();
        ...
        if (headingHierarchy.size > 0)
            semanticHtmlScore += 25;
        if (landmarkRegions.length > 0)
            semanticHtmlScore += 25;
        if (formFields > 0)
            semanticHtmlScore += 25;
        if (domModel.images.length > 0)
            semanticHtmlScore += 25;
        return {
            headingHierarchy,
    ...
    generateRecommendations(wcagResult, metrics) {
    ...
    calculateAccessibilityScore(wcagResult, metrics) {
        let score = 100;

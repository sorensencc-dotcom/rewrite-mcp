import type { DomModel, DomNode } from './dom.js';
...
export class WcagValidator {
  private readonly targetLevel: WcagLevel;
  private readonly domModel: DomModel;
  private readonly computedStyles?: ComputedStyle[];

  constructor(domModel: DomModel, computedStyles?: ComputedStyle[], targetLevel: WcagLevel = 'AA') {
  ...
  private checkContrast(): WcagIssue[] {
    const issues: WcagIssue[] = [];
    ...
    for (const style of this.computedStyles) {
      const ratio = this.calculateContrastRatio(
        style.computed.color ?? '#000000',
        style.computed['background-color'] ?? '#ffffff',
      );
  ...
  private checkKeyboardAccess(): WcagIssue[] {
    const issues: WcagIssue[] = [];

    // Walk DOM to find interactive elements without keyboard access
    const findInteractive = (node: DomNode, path: string = ''): void => {
  ...
  private checkNavigation(): WcagIssue[] {
    const issues: WcagIssue[] = [];
    ...
    // 2.4.1 Bypass Blocks
    const hasSkipLink = this.domModel.root.children.some(
      node => node.attributes['href']?.includes('#main') || node.attributes['href']?.includes('#content'),
    );
  ...
  private checkReadability(): WcagIssue[] {
    const issues: WcagIssue[] = [];
  ...
  private checkPredictability(): WcagIssue[] {
  ...
  private checkCompatibility(): WcagIssue[] {
    const issues: WcagIssue[] = [];

    // 4.1.2 Name, Role, Value
    // Check form inputs for labels
    const findUnlabeledInputs = (node: DomNode, path: string = ''): void => {
      const currentPath = `${path}/${node.tag}${node.id ? `#${node.id}` : ''}`;

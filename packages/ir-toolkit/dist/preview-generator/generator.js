export class PreviewGenerator {
    ...
    generate(ir) {
        ...
        const transformationNarrative = this.generateNarrative(ir, componentPreviews, overallScore);
        const keyHighlights = this.extractKeyHighlights(componentPreviews, layoutDiffs, designTokenDiffs);
    ...
    calculateOverallScore(componentPreviews, layoutDiffs, designTokenDiffs) {
    ...
    estimateComponentEffort(comp, improvements) {
        const complexityScore = comp.complexity ?? 0;
        const improvementCount = improvements.length;
        if (complexityScore < 30 && improvementCount < 2)
            return 'low';
        if (complexityScore > 70 && improvementCount > 3)
            return 'high';
        return 'medium';
    }
    estimateOverallEffort(componentPreviews) {
        ...
        if (highCount > 10)
            return 'enterprise';
        if (highCount > 5 || mediumCount > 15)
            return 'high';
        if (mediumCount > 5)
            return 'medium';
        return 'low';
    }
    categorizeComponents(componentPreviews) {
        const categories = {
            hero: [],
            forms: [],
            navigation: [],
        ...
        for (const comp of componentPreviews) {
            ...
            if (name.includes('hero'))
                categories.hero.push(comp);
            else if (name.includes('form') || name.includes('input'))
                categories.forms.push(comp);
    ...
    generateNarrative(ir, componentPreviews, score) {
    ...
    extractKeyHighlights(componentPreviews, layoutDiffs, designTokenDiffs) {
        const highlights = [];
        const topComponents = componentPreviews
            .sort((a, b) => b.improvementScore - a.improvementScore)
            .slice(0, 3);
        if (topComponents.length > 0) {
            highlights.push(`${topComponents.map(c => c.name).join(', ')} — highest improvement potential`);
        }
        if (layoutDiffs.length > 0) {
            highlights.push(`Layout system — ${layoutDiffs[0].rationale}`);
        }
        if (designTokenDiffs.length > 0) {
            highlights.push(`Design tokens — Standardize ${designTokenDiffs.map(d => d.category).join(', ')}`);
        }
...
export function generatePreview(ir, config) {

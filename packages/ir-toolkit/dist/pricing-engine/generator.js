export class PricingEngine {
    ...
    generate(gallery) {
        ...
        const hasAccessibilityWork = gallery.componentPreviews.some(c => c.improvements.some(i => i.aspect === 'accessibility'));
    ...
    calculateComponentCosts(gallery) {
        return gallery.componentPreviews.map(preview => {
            ...
            const totalCost = Math.round(baseCost * effortValue * (1 + complexityMult));
            return {
                id: preview.id,
                name: preview.name,
                category,
    ...
    determineCategoryForComponent(preview, gallery) {
        const categories = gallery.componentCategories;
        const name = preview.name.toLowerCase();
        if (categories.hero.some(c => c.id === preview.id))
            return 'hero';
        if (categories.forms.some(c => c.id === preview.id))
            return 'forms';
        if (categories.navigation.some(c => c.id === preview.id))
            return 'navigation';
        if (categories.cards.some(c => c.id === preview.id))
            return 'cards';
        if (categories.buttons.some(c => c.id === preview.id))
            return 'buttons';
        return 'other';
    }
    effortToMultiplier(effort) {
        switch (effort) {
            case 'low':
                return 1.0;
            case 'medium':
    ...
    calculateCostBreakdown(total, gallery) {
    ...
    calculateConfidence(gallery) {
        const categorizedComponents = Object.values(gallery.componentCategories).reduce((sum, arr) => sum + arr.length, 0);
    ...
    generateCustomizations(gallery) {
        ...
        const highComplexityComponents = gallery.componentPreviews.filter(c => c.currentComplexity > 70).length;
    ...
    generateSummary(total, tier, gallery) {
...
export function generatePricing(gallery, config) {

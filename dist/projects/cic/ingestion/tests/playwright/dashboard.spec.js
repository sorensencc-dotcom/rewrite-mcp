...
test_1.test.describe('CIC Observability Dashboard', () => {
    (0, test_1.test)('loads dashboard, regions, and initializes polling loop', async ({ page }) => {
        ...
        await (0, test_1.expect)(regionSelect).toBeVisible();
        await page.waitForFunction((sel) => sel.options.length > 0, await regionSelect.elementHandle());
        // Core DOM elements
        await (0, test_1.expect)(page.locator('#agent-grid')).toBeVisible();
        await (0, test_1.expect)(page.locator('#event-log')).toBeVisible();
        await (0, test_1.expect)(page.locator('#pipeline-diagram')).toBeVisible();
        // JS bootstrapped
        const polling = await page.evaluate(() => window.__CIC_POLLING_ACTIVE__);
        (0, test_1.expect)(polling).toBe(true);
    });
    (0, test_1.test)('renders all 5 core agents with valid states for a region', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        ...
        for (const agent of agents) {
            ...
            await (0, test_1.expect)(node).toBeVisible();
            const state = await node.getAttribute('data-state');
            (0, test_1.expect)(['ONLINE', 'DOWN', 'DEGRADED', 'PENDING']).toContain(state);
        }
    });
    (0, test_1.test)('event log shows max 50 entries and auto-trims', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        // Wait for log to populate
        await page.waitForSelector('.event-log-entry');
        const entries = page.locator('.event-log-entry');
        const count = await entries.count();
        (0, test_1.expect)(count).toBeLessThanOrEqual(50);
        // Validate timestamp format
        const first = await entries.first().innerText();
        (0, test_1.expect)(first).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
    });
    (0, test_1.test)('pipeline diagram renders all nodes and transitions animate', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        const nodes = ['INGEST', 'ENRICH', 'ORCHESTRATE', 'SYNTHESIZE', 'AUDIT'];
        for (const n of nodes) {
            await (0, test_1.expect)(page.locator(`[data-pipeline-node="${n}"]`)).toBeVisible();
        }
        ...
        (0, test_1.expect)(hasPulse).toBe(1);
    });
    (0, test_1.test)('visual regression: dashboard initial state', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        // Mask dynamic content to avoid false positives
        await (0, test_1.expect)(page).toHaveScreenshot('dashboard-initial.png', {
            mask: [
    ...
    (0, test_1.test)('visual regression: pipeline diagram nodes', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        const diagram = page.locator('#pipeline-diagram');
        await (0, test_1.expect)(diagram).toHaveScreenshot('pipeline-diagram.png');
    });
    (0, test_1.test)('handles agent timeout (AbortController 5s limit) on region proxy', async ({ page }) => {
        ...
        await (0, test_1.expect)(ingester).toHaveAttribute('data-state', /DOWN|DEGRADED|PENDING/, { timeout: 15000 });
        const logEntry = page.locator('.event-log-entry').first();
        await (0, test_1.expect)(logEntry).toContainText(/timeout|failed|error/i);
    });
    (0, test_1.test)('Intelligence Timeline renders recent events', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
    ...
    (0, test_1.test)('filters Intelligence Timeline events', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        await (0, test_1.expect)(page.locator('#timeline-body')).not.toContainText('Initializing timeline...', { timeout: 10000 });
        ...
        await (0, test_1.expect)(page.locator('#timeline-body')).toContainText('OVERRIDE', { timeout: 15000 });
        // 4. Type a garbage search and expect no matches
        await searchInput.fill('zxqjkwxyz');
        await (0, test_1.expect)(page.locator('#timeline-body')).toContainText('No events match the current filters.', { timeout: 5000 });
    });
    (0, test_1.test)('propagates correlation ID from manual action to timeline', async ({ page }) => {
        await page.goto('http://127.0.0.1:8080/dashboard?bypass_auth=true');
        ...
        (0, test_1.expect)(timelineText).toContain('OVERRIDE');
        ...
        (0, test_1.expect)(overrideLine).toBeDefined();
        // Shortened CID regex: [a-f0-9]{8} or similar
        (0, test_1.expect)(overrideLine).toMatch(/\[.{8}\]/i);
    });
});

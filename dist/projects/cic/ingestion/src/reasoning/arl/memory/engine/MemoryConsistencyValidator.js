...
class MemoryConsistencyValidator {
     ...
     */
    async validate(expansion, memory) {
        const violations = [];
        ...
        for (const claim of expansion.claims) {
            ...
            if (!entity)
                continue;
            const temporalViolations = this.checkTemporalConsistency(claim, entity, expansion);
        ...
        for (const claim of expansion.claims) {
            ...
            if (!entity)
                continue;
            const contradictions = this.checkContradictions(claim, entity, expansion);
        ...
        const alignmentScore = Math.max(0.2, baseScore - violationPenalty) * Math.max(0.5, temporalCoherence) * Math.max(0.5, narrativeCoherence);
        ...
        let approvalRecommendation = 'auto_approve';
     ...
     */
    checkTemporalConsistency(claim, entity, expansion) {
        const violations = [];
        ...
        if (claim.claimType === 'event') {
            ...
            const priorEvents = entity.events.filter((e) => new Date(e.timestamp) <= claimTime);
            ...
            if (priorEvents.length > 0) {
                ...
                const isDeath = lastPriorDesc.includes('died') ||
     ...
     */
    checkContradictions(claim, entity, expansion) {
        const violations = [];
            ...
            if (entity.attributes[attrKey]) {
                ...
                if (existingValue.toLowerCase() !== attrValue.toLowerCase()) {
                    violations.push({
                        ...
                        suggestedResolution: 'Verify which value is correct; may require separate validation',
                    });
                }
            }
            // Check for conflicts within this expansion (two claims about same attribute)
            const otherAttributeClaims = expansion.claims.filter((c) => c.entityId === claim.entityId &&
        ...
        if (claim.claimType === 'relationship') {
            ...
            const existingRel = entity.relationships.find((r) => r.relatedEntityId === relatedId);
            if (existingRel &&
     ...
     */
    checkMissingContext(claim) {
        const missing = [];
        // Spatial context
        if (claim.statement.toLowerCase().includes('location') &&
        ...
        if (claim.claimType === 'event' &&
            ...
            missing.push(`Event claim too brief, may lack context: "${claim.statement}"`);
        }
        // Causal context
        if (claim.statement.includes('because') &&
    ...
    getConsistencyStats() {
        ...
        const maxDrift = Math.max(...results.map((r) => Math.abs(r.driftVector)));

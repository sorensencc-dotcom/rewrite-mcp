...
exports.ArlSelfDiagnostics = void 0;
 ...
 */
class ArlSelfDiagnostics {
    constructor(expectedWeights = {
        coherence: 0.20,
        semantic: 0.25,
        temporal: 0.20,
        causal: 0.15,
        narrative: 0.20,
    }, config = ThresholdConfig_1.DEFAULT_THRESHOLDS) {
        this.subsystemScores = new Map();
        this.driftHistory = [];
        this.thresholdChecks = new Map();
     ...
     */
    recordSubsystemScore(subsystem, score) {
     ...
     */
    recordThresholdDecision(rejectCode, wasEscalated, wasOverridden) {
        const key = rejectCode || 'E000_accept'; // Group by reject code or accept
    ...
    runDiagnostics() {
        ...
        const reasoningIntegrity = this.computeReasoningIntegrity(subsystemHealthChecks, driftAnalysis, weightingValidation, thresholdCalibration);
        return {
            timestamp: new Date(),
            phaseId: '7.14',
            reasoningIntegrity,
        ...
        for (const [subsystem, scores] of this.subsystemScores) {
            ...
            if (recent.length > 0 && older.length > 0) {
                ...
                if (recentMean < olderMean - 0.05) {
                    issues.push(`Deteriorating trend: ${olderMean.toFixed(3)} → ${recentMean.toFixed(3)}`);
                    status = 'degraded';
                    score -= 0.15;
                }
            }
    ...
    analyzeDrift() {
        ...
        const driftVariance = driftMagnitudes.reduce((sum, d) => sum + Math.pow(d - driftMean, 2), 0) /
        ...
        const anomalies = this.driftHistory.filter((d) => Math.abs(d.magnitude - driftMean) > 2 * stdDev);
    ...
    calibrateThresholds() {
        return {
            compositeReasoningMin: this.analyzeThresholdQuality('composite_reasoning', this.config.compositeReasoningMin),
            confidenceMin: this.analyzeThresholdQuality('confidence', this.config.confidenceMin),
            driftMaxMagnitude: this.analyzeThresholdQuality('drift', this.config.driftMaxMagnitude),
            contradictionSeverityMax: this.analyzeThresholdQuality('contradiction', this.config.contradictionSeverityMax),
        };
    }
    /**
     * Analyze if a single threshold is well-calibrated
     */
    analyzeThresholdQuality(name, threshold) {
     ...
     */
    computeReasoningIntegrity(subsystemHealth, driftAnalysis, weighting, calibration) {
        const issues = [];
        const recommendations = [];
        // Subsystem health score
        const subsystemHealthScore = subsystemHealth.length > 0
        ...
        if (driftAnalysis.seasonality) {
            recommendations.push('Seasonal pattern detected in drift; adjust expectations or add seasonal handling');
        }
        ...
        const overall = subsystemHealthScore * 0.3 +
...
exports.ArlSelfDiagnostics = ArlSelfDiagnostics;
//# sourceMappingURL=ArlSelfDiagnostics.js.map
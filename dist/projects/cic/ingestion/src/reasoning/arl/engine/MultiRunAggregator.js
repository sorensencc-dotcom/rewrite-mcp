...
class MultiRunAggregator {
    aggregate(runs) {
        ...
        const driftAvg = runs.reduce((sum, r) => sum + r.driftScore, 0) / runs.length;
        const contradictionAvg = runs.reduce((sum, r) => sum + r.contradictionScore, 0) / runs.length;
        const semanticAvg = runs.reduce((sum, r) => sum + r.semanticScore, 0) / runs.length;
    ...
    appendRun(existing, newRun) {
        ...
        const driftAvg = (existing.rollingDriftAverage * existing.runCount +
            newRun.driftScore) /
            newCount;
        const contradictionAvg = (existing.rollingContradictionAverage * existing.runCount +
            newRun.contradictionScore) /
            newCount;
        const semanticAvg = (existing.rollingSemanticAverage * existing.runCount +
    ...
    computeStabilityScore(driftAvg, contradictionAvg) {
    ...
    detectTrend(runs) {
        ...
        const firstDriftAvg = firstHalf.reduce((sum, r) => sum + r.driftScore, 0) / firstHalf.length;
        const secondDriftAvg = secondHalf.reduce((sum, r) => sum + r.driftScore, 0) / secondHalf.length;

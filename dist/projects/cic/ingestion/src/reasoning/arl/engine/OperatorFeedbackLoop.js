...
exports.OperatorFeedbackLoop = void 0;
class OperatorFeedbackLoop {
    constructor() {
        this.maxAdjustmentPerRun = 0.1;
        this.adjustmentBounds = { min: -0.2, max: 0.2 };
    }
    computeAdjustments(feedback) {
        ...
        for (const fb of feedback) {
            ...
            else if (fb.originalVerdict === 'ACCEPT' &&
        ...
        return {
            ...
            thresholdAdjustments: {
                ...
                contradiction: this.boundAdjustment(thresholdAdjustments.contradiction),
            },
            narrativeRiskAdjustment: this.boundAdjustment(narrativeRiskAdjustment),
        };
    }
    applyAdjustments(config, adjustment) {
        const result = { ...config };
        if (adjustment.thresholdAdjustments.composite !== undefined) {
            result.compositeReasoningMin = Math.max(0, Math.min(1, result.compositeReasoningMin +
                adjustment.thresholdAdjustments.composite));
        }
        if (adjustment.thresholdAdjustments.confidence !== undefined) {
            result.confidenceMin = Math.max(0, Math.min(1, result.confidenceMin + adjustment.thresholdAdjustments.confidence));
        }
        if (adjustment.thresholdAdjustments.drift !== undefined) {
            result.driftMaxMagnitude = Math.max(0, Math.min(1, result.driftMaxMagnitude + adjustment.thresholdAdjustments.drift));
        }
        if (adjustment.thresholdAdjustments.contradiction !== undefined) {
            result.contradictionSeverityMax = Math.max(0, Math.min(1, result.contradictionSeverityMax +
                adjustment.thresholdAdjustments.contradiction));
        }
        return result;
    }
    computeAdjustmentDirection(feedback) {
        if (feedback.originalVerdict === 'REJECT' &&
            feedback.operatorVerdict === 'ACCEPT') {
            return 1;
        }
        else if (feedback.originalVerdict === 'ACCEPT' &&
    ...
    boundAdjustment(adjustment) {
        return Math.max(this.adjustmentBounds.min, Math.min(this.adjustmentBounds.max, adjustment));
    }
}
exports.OperatorFeedbackLoop = OperatorFeedbackLoop;
//# sourceMappingURL=OperatorFeedbackLoop.js.map
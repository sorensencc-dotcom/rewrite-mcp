...
exports.AmbStrategicPlanner = void 0;
const node_crypto_1 = __importDefault(require("node:crypto"));
class AmbStrategicPlanner {
    constructor() {
        this.defaultHorizon = 3;
    }
    /**
     * Generate a strategic plan from current intents, bundles, and memory.
     */
    generatePlan(runId, intents, bundles, memory) {
        ...
        return {
            plan_id: `plan-${node_crypto_1.default.randomUUID().substring(0, 8)}`,
            run_id: runId,
            timestamp: now,
            version: "v0.1.0",
     ...
     */
    sequenceSteps(patterns, bundles, intents) {
        const planned = [];
        let step = 1;
     ...
     */
    projectImpact(plannedIntents, bundles, memory) {
     ...
     */
    collectPolicyConstraints(intents) {
        const constraints = new Set();
        for (const intent of intents) {
            if (intent.policy_alignment.forbidden_domain)
                constraints.add("forbidden_domain");
            if (intent.policy_alignment.operator_required)
                constraints.add("operator_required");
            if (intent.policy_alignment.lineage_required)
                constraints.add("lineage_required");
            if (intent.policy_alignment.rl_dependent)
                constraints.add("rl_dependent");
        }
        return Array.from(constraints);
    }
}
exports.AmbStrategicPlanner = AmbStrategicPlanner;
//# sourceMappingURL=ambStrategicPlanner.js.map
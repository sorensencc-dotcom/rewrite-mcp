...
exports.AmbIntentBundler = void 0;
const node_crypto_1 = __importDefault(require("node:crypto"));
class AmbIntentBundler {
    /**
     * Group intents into coherent bundles by domain affinity.
     */
    bundleIntents(runId, intents) {
        if (intents.length === 0)
            return [];
     ...
     */
    classifyBundleType(intent) {
        const domains = intent.target_domains || {};
        if (domains.ckg_graph)
            return "graph_cleanup";
        if (domains.mas_topology)
            return "mas_stability";
        if (domains.rl_fusion)
            return "tenant_redesign";
        if (domains.cic_config)
            return "planner_tuning";
        // Fallback: infer from intent_type
        const type = (intent.intent_type || "").toLowerCase();
        if (type.includes("distill") || type.includes("graph"))
            return "graph_cleanup";
        if (type.includes("mas") || type.includes("stability"))
            return "mas_stability";
        if (type.includes("fusion") || type.includes("redesign"))
            return "tenant_redesign";
        if (type.includes("planner") || type.includes("tuning"))
            return "planner_tuning";
     ...
     */
    formBundle(runId, bundleType, intents, timestamp) {
        ...
        return {
            bundle_id: `bundle-${bundleType}-${node_crypto_1.default.randomUUID().substring(0, 8)}`,
            run_id: runId,
            timestamp,
     ...
     */
    generateBundleSummary(bundleType, intents) {
...
exports.AmbIntentBundler = AmbIntentBundler;
//# sourceMappingURL=ambIntentBundler.js.map
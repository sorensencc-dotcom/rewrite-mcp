...
exports.AmbMemoryStore = void 0;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const node_crypto_1 = __importDefault(require("node:crypto"));
class AmbMemoryStore {
    constructor(baseDir) {
        this.memoryDir = baseDir || node_path_1.default.resolve(process.cwd(), "projects/cic/evolution/data/amb/memory");
    ...
    loadLatestSnapshot() {
        ...
        const latestPath = node_path_1.default.join(this.memoryDir, files[0]);
        try {
            const raw = node_fs_1.default.readFileSync(latestPath, "utf8");
    ...
    recordRun(params) {
        ...
        const intentRecords = params.intents.map(i => ({
        ...
        const proposalRecords = params.proposals.map(p => ({
        ...
        const snapshot = {
            snapshot_id: `mem-${node_crypto_1.default.randomUUID().substring(0, 8)}`,
            timestamp: now,
    ...
    saveSnapshot(snapshot) {
        ...
        const filePath = node_path_1.default.join(this.memoryDir, filename);
    ...
    getProposalSuccessRate() {
        const snapshot = this.loadLatestSnapshot();
        if (!snapshot || snapshot.proposals.length === 0)
            return 0;
    ...
    listSnapshotFiles() {
        try {
            return node_fs_1.default.readdirSync(this.memoryDir).filter(f => f.startsWith("memory_") && f.endsWith(".json"));
        }
        catch {
            return [];
        }
...
exports.AmbMemoryStore = AmbMemoryStore;
//# sourceMappingURL=ambMemoryStore.js.map
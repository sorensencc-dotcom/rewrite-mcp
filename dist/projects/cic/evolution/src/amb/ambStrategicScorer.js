...
exports.AmbStrategicScorer = void 0;
class AmbStrategicScorer {
    constructor(memory) {
        this.memory = memory;
    }
    /**
     ...
     */
    computeHistoricalBoost(intentType) {
        if (!this.memory || this.memory.proposals.length === 0)
            return 1.0;
        // Find past intents of this type
        const pastIntents = this.memory.intents.filter(i => i.intent_type === intentType);
        if (pastIntents.length === 0)
            return 1.0;
        // Find proposals linked to those intents
        const intentIds = new Set(pastIntents.map(i => i.intent_id));
        const linkedProposals = this.memory.proposals.filter(p => p.source_intent_id && intentIds.has(p.source_intent_id));
     ...
     */
    computeOperatorBurden(intent) {
        const actions = intent.constraints?.required_operator_actions || [];
        return 1 + actions.length;
    }
}
exports.AmbStrategicScorer = AmbStrategicScorer;
//# sourceMappingURL=ambStrategicScorer.js.map
...
exports.graphBuilder = exports.GraphBuilder = void 0;
...
const defaultGraphPath = path_1.default.resolve(__dirname, "../../data/graph-store.json");
class GraphBuilder {
    constructor() {
        // Scoped structures: tenantId -> Map/Array
        this.tenantDocuments = new Map();
        this.tenantEntities = new Map();
        this.tenantDocEntityOccurrences = new Map();
        this.tenantEntityDocOccurrences = new Map();
        this.tenantRelationships = new Map();
        this.tenantCrossDocLinks = new Map();
    ...
    getEntityDocOccurrences(tenantId) {
    ...
    getTenantPath(filePath, tenantId) {
        ...
        return path_1.default.resolve(__dirname, `../../data/tenants/${tenantId}/graph-store.json`);
    }
    load(filePath, tenantId = "default") {
        ...
        try {
            ...
            metrics_collector_js_1.metricsCollector.recordGraphLoad(Date.now() - tStart);
        }
        catch (err) {
    ...
    async createSnapshot(tag, tenantId = "default") {
        const tStart = Date.now();
        const timestampStr = new Date().toISOString().replace(/[:.]/g, "-");
        const filename = `graph_snapshot_${timestampStr}${tag ? "_" + tag : ""}.json`;
        const snapshotDir = tenantId === "default"
            ? path_1.default.resolve(__dirname, "../../data/snapshots")
            : path_1.default.resolve(__dirname, `../../data/tenants/${tenantId}/snapshots`);
        if (!fs_1.default.existsSync(snapshotDir)) {
            fs_1.default.mkdirSync(snapshotDir, { recursive: true });
        }
        const snapshotPath = path_1.default.join(snapshotDir, filename);
        this.save(snapshotPath, tenantId);
        try {
            const duration = Date.now() - tStart;
            const sizeBytes = fs_1.default.existsSync(snapshotPath) ? fs_1.default.statSync(snapshotPath).size : 0;
            metrics_collector_js_1.metricsCollector.recordGraphSnapshot(tag || "manual", sizeBytes, duration);
        }
        catch {
            // ignore
        }
    ...
    sliceAtDate(dateXStr, tenantId = "default") {
    ...
    addDocumentGraph(doc, links, tenantId = "default") {
        if (!doc || !doc.docId) {
        ...
        for (const rel of docRelationships) {
            ...
            if (!subjectId && rel.subject) {
                const sName = rel.subject;
                const matched = docEntities.find(e => e.name === sName);
                if (matched)
                    subjectId = matched.id;
            }
            if (!objectId && rel.object) {
                const oName = rel.object;
            ...
            if (!subjectId || !objectId)
                continue;
            const exists = relationships.some(r => r.subjectId === subjectId && r.objectId === objectId && r.predicate === rel.predicate);
            if (!exists) {
                relationships.push({
                    subjectId,
                    objectId,
    ...
    getEntityNeighborhood(entityId, tenantId = "default") {
        const entities = this.getEntities(tenantId);
    ...
    getDocumentNeighborhood(docId, tenantId = "default") {
        const documents = this.getDocuments(tenantId);
        const entities = this.getEntities(tenantId);
        const docEntityOccurrences = this.getDocEntityOccurrences(tenantId);
        const crossDocLinks = this.getCrossDocLinks(tenantId);
...
exports.graphBuilder = new GraphBuilder();

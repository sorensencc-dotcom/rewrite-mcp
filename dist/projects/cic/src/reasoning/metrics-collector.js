...
exports.metricsCollector = exports.MetricsCollector = exports.SLO_TARGETS = void 0;
exports.SLO_TARGETS = {
...
class MetricsCollector {
    constructor() {
        ...
        this.extractorLatencyBuffer = [];
        // Rolling Backlog Age buffer
        this.backlogAgeBuffer = [];
    ...
    recordIngestion(durationMs, latencies) {
        const now = Date.now();
    ...
    recordRAGQuery(durationOrStages, stagesOrEvidence, evidenceOrContradictions, contradictions) {
        const now = Date.now();
    ...
    recordSafeguardTrigger(triggerType, reason, details) {
    ...
    recordRTKIntervention(action, outcome) {
    ...
    evaluateSLOs() {
        ...
        const extStatus = p95Extractor <= exports.SLO_TARGETS.ingestion.p95ExtractorLatencyMs ? "pass" : "fail";
        evals.push({
            metric: "Ingestion Extractor p95 Latency",
            target: `< ${exports.SLO_TARGETS.ingestion.p95ExtractorLatencyMs}ms`,
        ...
        const failStatus = failPct <= exports.SLO_TARGETS.ingestion.failureRatePct ? "pass" : "fail";
        evals.push({
            metric: "Ingestion Failure Rate",
            target: `< ${exports.SLO_TARGETS.ingestion.failureRatePct}%`,
        ...
        const backlogStatus = maxBacklog <= exports.SLO_TARGETS.ingestion.backlogAgeSec ? "pass" : "fail";
        evals.push({
            metric: "Ingestion Backlog Age",
            target: `< ${exports.SLO_TARGETS.ingestion.backlogAgeSec}s`,
            actual: `${maxBacklog}s`,
            status: backlogStatus
        });
        // 4. Vector p95 Hybrid Search Latency (< 120ms)
        const p95Vector = this.getPercentile(this.vectorLatencyBuffer, 95);
        const vecStatus = p95Vector <= exports.SLO_TARGETS.vectorIndex.p95HybridSearchLatencyMs ? "pass" : "fail";
        evals.push({
            metric: "Vector Hybrid Search p95 Latency",
            target: `< ${exports.SLO_TARGETS.vectorIndex.p95HybridSearchLatencyMs}ms`,
        ...
        const througputStatus = (this.peakUpsertThroughput >= exports.SLO_TARGETS.vectorIndex.upsertThroughputPerSec || this.totalUpserts === 0) ? "pass" : "fail";
        evals.push({
            metric: "Vector Peak Upsert Throughput",
            target: `> ${exports.SLO_TARGETS.vectorIndex.upsertThroughputPerSec}/sec`,
            actual: `${this.peakUpsertThroughput}/sec`,
            status: througputStatus
        });
        // 6. Persistent Graph Startup Load (< 500ms)
        const loadStatus = this.startupLoadTimeMs <= exports.SLO_TARGETS.persistentGraph.startupLoadTimeMs ? "pass" : "fail";
        evals.push({
            metric: "Graph Startup Load Duration",
            target: `< ${exports.SLO_TARGETS.persistentGraph.startupLoadTimeMs}ms`,
        ...
        const snapStatus = snapDuration <= exports.SLO_TARGETS.persistentGraph.snapshotCreationTimeMs ? "pass" : "fail";
        evals.push({
            metric: "Graph Snapshot Creation Latency",
            target: `< ${exports.SLO_TARGETS.persistentGraph.snapshotCreationTimeMs}ms`,
        ...
        const mergeStatus = avgMerge <= exports.SLO_TARGETS.persistentGraph.lineageMergeMs ? "pass" : "fail";
        evals.push({
            metric: "Graph Avg Lineage Merge Duration",
            target: `< ${exports.SLO_TARGETS.persistentGraph.lineageMergeMs}ms`,
            actual: `${avgMerge.toFixed(2)}ms`,
            status: mergeStatus
        });
        // 9. RAG Reasoning p95 Cycle (< 1.2s)
        const p95Reasoning = this.getPercentile(this.ragLatencyBuffer, 95);
        const reasoningStatus = p95Reasoning <= exports.SLO_TARGETS.ragReasoning.p95ReasoningMs ? "pass" : "fail";
        evals.push({
            metric: "RAG Reasoning p95 Cycle",
            target: `< ${exports.SLO_TARGETS.ragReasoning.p95ReasoningMs}ms`,
        ...
        const contrStatus = contrRate <= exports.SLO_TARGETS.ragReasoning.contradictionRatePct ? "pass" : "fail";
        evals.push({
            metric: "RAG Contradiction Rate",
            target: `< ${exports.SLO_TARGETS.ragReasoning.contradictionRatePct}%`,
        ...
        const evidenceStatus = avgEvidence <= exports.SLO_TARGETS.ragReasoning.maxEvidenceBundleSize ? "pass" : "fail";
        evals.push({
            metric: "RAG Evidence Bundle Avg Size",
            target: `< ${exports.SLO_TARGETS.ragReasoning.maxEvidenceBundleSize} items`,
        ...
        const triggersStatus = triggersCount === exports.SLO_TARGETS.automation.safeguardTriggers ? "pass" : "fail";
        evals.push({
            metric: "RTK Active Safeguard Triggers",
            target: `== ${exports.SLO_TARGETS.automation.safeguardTriggers}`,
        ...
        const driftStatus = avgDrift <= exports.SLO_TARGETS.automation.dryRunDriftPct ? "pass" : "fail";
        evals.push({
            metric: "RTK Dry-Run Avg Memory/State Drift",
            target: `< ${exports.SLO_TARGETS.automation.dryRunDriftPct}%`,

...
const retrieval_planner_js_1 = require("./retrieval-planner.js");
const evidence_collector_js_1 = require("./evidence-collector.js");
const reason_trace_js_1 = require("./reason-trace.js");
...
class ReasoningOrchestrator {
    async reason(query, context) {
        ...
        const stageLatenciesMs = {};
        // 1. Planning Stage
        const tPlanStart = Date.now();
        const plan = retrieval_planner_js_1.retrievalPlanner.plan(query, context);
        stageLatenciesMs["planning"] = Date.now() - tPlanStart;
        // 2. Collection Stage
        const tCollectStart = Date.now();
        const evidence = await evidence_collector_js_1.evidenceCollector.collect(plan);
        ...
        const traceId = "trc_" + crypto_1.default.randomUUID().replace(/-/g, "").slice(0, 16);
        const trace = {
            traceId,
            query,
            plan,
        ...
        reason_trace_js_1.reasonTraceManager.save(trace);
        // Record telemetry metrics
        const duration = Date.now() - startTime;
        metrics_collector_js_1.metricsCollector.recordRAGQuery(duration, Object.keys(stageLatenciesMs).length, evidence.length, contradictionsDetected.length);
        return trace;
    }
    ...
    async resolvePMSTemplate(templateId, vars) {
        try {
            return await composer_js_1.pmsComposer.resolve(templateId, vars);
        }
        catch {
            return { prompt: `Fallback prompt compiling ${templateId} with query: ${vars.query}` };
        }
    }
    simulateFinalSynthesis(query, evidence, contradictions) {

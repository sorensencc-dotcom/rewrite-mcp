...
exports.MeeScheduler = void 0;
class MeeScheduler {
    constructor(jobs, runs, engine, workspacePath, concurrencyLimit = 2) {
        this.jobs = jobs;
        this.runs = runs;
        this.engine = engine;
    ...
    async tick() {
            ...
            if (this.activeJobs.has(job.id)) {
                ...
                if (run) {
                    if (run.status === "completed") {
                        ...
                        this.engine.addMemory("job", job.id, job.runId, ["success", "completed", "phase40"], `Job ${job.id} completed successfully`, `Scheduler finished job ${job.id}`);
                    }
        ...
        const eligibleJobs = this.jobs.list().filter((j) => j.status === "pending" || j.status === "paused" || j.status === "running");
        // Filter by dependencies: all dependsOnJobIds must be "completed"
        const dependencyCheckedJobs = eligibleJobs.filter((job) => {
            if (!job.dependsOnJobIds || job.dependsOnJobIds.length === 0)
                return true;
        ...
        if (this.activeJobs.size >= this.concurrencyLimit && sortedJobs.length > 0) {
            ...
            if (nextPending) {
                ...
                const runningSortedByPriority = [...runningActiveJobs].sort((a, b) => (a.priority ?? 0) - (b.priority ?? 0));
                ...
                if (lowestRunning && (lowestRunning.priority ?? 0) < priorityPending) {
                    ...
                    this.engine.addMemory("job", lowestRunning.id, lowestRunning.runId, ["scheduler", "preemption", "phase40"], `Job ${lowestRunning.id} preempted by job ${nextPending.id}`, `Preempted job due to priority queue threshold.`);
                }
            }
        }
...
exports.MeeScheduler = MeeScheduler;
//# sourceMappingURL=mee-scheduler.js.map
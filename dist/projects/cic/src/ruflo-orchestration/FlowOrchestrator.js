...
const CachedAgentClient_js_1 = require("./CachedAgentClient.js");
/**
 * FlowOrchestrator executes flow templates with agent coordination
 */
class FlowOrchestrator extends events_1.EventEmitter {
    constructor(config) {
        super();
        ...
        if (config.cache) {
            ...
            for (const [name, agent] of Object.entries(config.agents)) {
                this.agents[name] = new CachedAgentClient_js_1.CachedAgentClient({
                    agent,
                    cache: config.cache,
                    ttl: config.cacheTtl,
                });
...
exports.default = FlowOrchestrator;

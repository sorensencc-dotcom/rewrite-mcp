...
exports.specRegistry = exports.SpecRegistry = void 0;
...
class SpecRegistry {
    constructor(specsDir = path_1.default.resolve(__dirname, "../../../cic-specs")) {
        this.skills = new Map();
    ...
    loadAll() {
        try {
            if (!fs_1.default.existsSync(this.specsDir)) {
                fs_1.default.mkdirSync(this.specsDir, { recursive: true });
            }
            const files = fs_1.default.readdirSync(this.specsDir).filter(f => f.endsWith(".yaml") || f.endsWith(".yml"));
            for (const file of files) {
                const fullPath = path_1.default.join(this.specsDir, file);
            ...
            try {
                const logDir = path_1.default.resolve(__dirname, "../../../data/telemetry");
                const filePath = path_1.default.join(logDir, "violations.jsonl");
                if (fs_1.default.existsSync(filePath)) {
                    const rawLines = fs_1.default.readFileSync(filePath, "utf-8").trim().split("\n");
    ...
    evaluateInstincts(pipeline, docType, sourceFormat, tenantId = "default", region = "us-east-1") {
        ...
        const activePatches = patch_loader_js_1.patchLoader.listPatches("active");
        const canaryPatches = patch_loader_js_1.patchLoader.listPatches("canary").filter(p => {
...
exports.specRegistry = new SpecRegistry();
exports.specRegistry.loadAll();

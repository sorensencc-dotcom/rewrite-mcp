...
class APRMemoryIntegration {
    ...
    async getPlanningContext() {
        ...
        const historicalSuccessRates = this.extractSuccessRates(await substrate.query({ event_type: "PLATFORM_EXTRACTION", limit: 1000 }));
     ...
     */
    async estimateTaskDuration(taskType, platform) {
        const substrate = this.memory.getSubstrate();
        ...
        const durations = events
            .map((e) => (e.payload.duration_ms || 0) / 1000 / 60)
            .filter(d => d > 0)
            .sort((a, b) => a - b);
    ...
    async getAgentRecommendations() {
        ...
        const recommendations = agentEvents.map((evt) => {
    ...
    identifyFailureClusters(summary) {
        const clusters = [];
        // Check for rate limiting issues
        if (summary.proposals_for_arps.some((p) => p.title.toLowerCase().includes("rate limit"))) {
            clusters.push({
        ...
        if (summary.proposals_for_arps.some((p) => p.title.toLowerCase().includes("reliability"))) {
            clusters.push({
    ...
    recommendApproaches(successRates) {
...
//# sourceMappingURL=memory-apr-integration.js.map